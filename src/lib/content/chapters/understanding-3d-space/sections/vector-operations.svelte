...
>
  ...
  <div class="readable-content">
    <ContentCard blockType="section-header-block">
      ...
      <p>
        Vectors often need to be combined or manipulated to understand physical
        situations. For example, how do multiple forces acting on an object
        combine? Or how do we describe a change in velocity? This section
        introduces the fundamental operations used to work with vectors:
        addition, subtraction, scalar multiplication, and the concepts of the
        dot and cross products. We'll look at both the visual intuition and the
        precise calculations using components.
      </p>
      <Scenario>
        <p data-speaker="Surya">
          <span class="speaker"><strong>Surya:</strong></span> Okay, vectors
          describe movement like <Katex>{"\\vec{v}"}</Katex> or forces like <Katex
            >{"\\vec{F}"}</Katex
          >. But what if there are *multiple* forces, or something changes speed
          *and* direction?
        </p>
        <p data-speaker="Leo">
          <span class="speaker"><strong>Leo:</strong></span> Great question! That's
          exactly why we need vector operations. They give us the mathematical tools
          to combine vectors (like adding forces) or scale them (like relating force
          to acceleration), and even explore relationships *between* vectors.
        </p>
      </Scenario>
    </ContentCard>
    ...
    <ContentCard blockType="concept-block">
      ...
      <h5>Concept</h5>
      <p>
        Subtracting a vector <Katex>{"\\vec{B}"}</Katex> from <Katex
          >{"\\vec{A}"}</Katex
        > is the same as adding the negative of <Katex>{"\\vec{B}"}</Katex> (a vector
      ...
      <p>
        Graphically, subtraction <Katex>{"\\vec{A} - \\vec{B}"}</Katex> is often
        visualized by placing the vectors tail-to-tail. The resultant vector points
        from the head of <Katex>{"\\vec{B}"}</Katex> to the head of <Katex
          >{"\\vec{A}"}</Katex
        >. This shows the change needed to get from <Katex>{"\\vec{B}"}</Katex> to
        <Katex>{"\\vec{A}"}</Katex> (since <Katex
          >{"\\vec{B} + (\\vec{A} - \\vec{B}) = \\vec{A}"}</Katex
        >).
      </p>
      <h5>Use Cases</h5>
      <ul>
        <li>
          ...
          to this): <Katex>{"\\Delta\\vec{v} = \\vec{v}_f - \\vec{v}_i"}</Katex
          >.
        </li>
    ...
    <ContentCard blockType="concept-block">
      ...
      <h4>Concept</h4>
      <p>
        Multiplying a vector <Katex>{"\\vec{A}"}</Katex> by a scalar (a regular number)
        <Katex>c</Katex> multiplies each component by <Katex>c</Katex>:
        <Katex displayMode>
      ...
      <ul>
        <li>
          If <Katex>{"c > 0"}</Katex>, the vector's magnitude is scaled by <Katex
            >c</Katex
          >, and its direction remains the same.
        </li>
        <li>
          If <Katex>{"c < 0"}</Katex>, the vector's magnitude is scaled by <Katex
            >{"\\|c\\|"}</Katex
          >, and its direction is reversed (flipped 180°).
        </li>
        <li>
          If <Katex>{"c = 0"}</Katex>, the result is the zero vector <Katex
            >{"\\langle 0, 0, 0 \\rangle"}</Katex
          >.
        </li>
      </ul>
      <h4>Utility</h4>
      <ul>
        <li>
          ...
          mass (<Katex>{"\\vec{F} = m\\vec{a}"}</Katex>).
        </li>
        <li>
          Relating <Keyword term="velocity" /> and <Keyword term="momentum" /> via
          mass (<Katex>{"\\vec{p} = m\\vec{v}"}</Katex>).
        </li>
        <li>
          Creating <Keyword term="unit vectors" />: A unit vector <Katex
            >{"\\hat{u}"}</Katex
          > in the same direction as <Katex>{"\\vec{u}"}</Katex> is found by <Katex
            >{"\\hat{u} = \\frac{1}{\\|\\vec{u}\\|}\\vec{u}"}</Katex
          >. This involves scalar multiplication by <Katex
            >{"1/\\|\\vec{u}\\|"}</Katex
          >.
        </li>
      </ul>
    ...
    <ContentCard blockType="concept-block">
      ...
      <p>
        The dot product combines two vectors to produce a single scalar value.
        It relates to how much one vector points along the direction of another.
      </p>
      <h4>Concept</h4>
      <p>
      ...
      <ul>
        <li>
          If the vectors are perpendicular (<Katex
            >{"\\theta = 90^\\circ"}</Katex
      ...
      <p>
        ...
        concept now will be essential when we delve into energy and other
        advanced topics where these applications are vital.
      </p>
    </ContentCard>
    ...
    <ContentCard blockType="concept-block">
      ...
      <p>
        ...
        *vector* that is perpendicular to both original vectors. This operation
      ...
      <ul>
        ...
        <li>
          ...
          />. (Point fingers of right hand along <Katex>{"\\vec{A}"}</Katex>,
          curl towards <Katex>{"\\vec{B}"}</Katex>; thumb points in direction of <Katex
      ...
      <ul>
        <li>
          ...
          (<Katex>{"\\sin 0^\\circ = \\sin 180^\\circ = 0"}</Katex>).
        </li>
        <li>
        ...
        <li>
          The component calculation is more complex and involves determinants.
          We will introduce the full component formula when its specific
          applications arise in later chapters.
        </li>
      </ul>
      <h4>Utility</h4>
      <p>
    ...
    <ContentCard blockType="conclusion-block">
      ...
      <p>
        Mastering vector addition, subtraction, and scalar multiplication is
        essential for combining and scaling physical quantities like
        displacement, velocity, and force. This section has laid the groundwork
        for these core operations.
      </p>
      <p>
        We also introduced the concepts of the Dot Product (yielding a scalar
        related to parallel alignment) and the Cross Product (yielding a vector
        related to perpendicularity and area). While we've touched upon their
        definitions and properties, their true power and detailed applications
        in calculating Work, Torque, Angular Momentum, and more will be explored
        in subsequent chapters. For now, focus on the component-wise
        calculations for addition, subtraction, and scalar multiplication, and
        the geometric meanings of all operations.
      </p>
    </ContentCard>
