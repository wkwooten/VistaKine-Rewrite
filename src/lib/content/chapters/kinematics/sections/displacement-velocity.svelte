...
>
  ...
  <div class="readable-content">
    <ContentCard blockType="section-header-block">
      ...
      <p>
        Welcome to Kinematics! Our first step is learning to describe *how*
        things move. It might seem simple, but physics requires precision. Just
        saying something moved "fast" or "far" isn't enough. We need to
        distinguish between the total path covered and the net change in
        location, and between raw speed and speed *with direction*. This section
        introduces the fundamental concepts of <Keyword term="Displacement" /> vs.
      ...
      <Scenario>
        ...
        <p data-speaker="Prof. Astra">
          Precisely! Its net change in position, its <Keyword
            term="Displacement"
          />, is zero! See the difference? One is just length, the other
          includes direction and tells us the *overall* change in location. We
          need both concepts, along with their time-based cousins, Speed and
          Velocity, to truly understand motion.
        </p>
      </Scenario>
    </ContentCard>
    ...
    <ContentCard blockType="concept-block">
      ...
      <dl class="definition-list">
        ...
        <dd>
          A <Keyword term="Scalar" /> quantity representing the total path length
          covered by an object during its motion. It only has magnitude (e.g., 10
          meters). Think of the odometer in a car – it only counts up, regardless
          of direction.
        </dd>
        <dt>
          <strong
            ><Keyword term="Displacement" /> (<Katex
              >{"`\\Delta \\vec{r}`"}</Katex
            >):</strong
          >
        </dt>
      ...
      <FormulaAccordion title="Displacement Formula">
        <p>
          The displacement vector <Katex>{"`\\Delta \\vec{r}`"}</Katex> is found
          by subtracting the initial position vector <Katex
            >{"`\\vec{r}_i`"}</Katex
          > from the final position vector <Katex>{"`\\vec{r}_f`"}</Katex>:
        </p>
        <Katex displayMode
      ...
      <p>
        <strong>Example Revisited:</strong> The drone flying 10m out and 10m
        back covers a distance <Katex>d = 20</Katex> m. However, since <Katex
    ...
    <ContentCard blockType="concept-block">
      ...
      <p>
        ...
        distinguish between how fast an object is moving and how fast its
        position is changing *in a specific direction*.
      </p>
      <dl class="definition-list">
        <dt>
          <strong
        ...
        <dd>
          A <Keyword term="Scalar" /> quantity representing how fast an object is
          moving. Average speed is calculated as total distance divided by total
          time (<Katex>{"`\\Delta t`"}</Katex>). Instantaneous speed is the
          magnitude of the instantaneous velocity vector.
        </dd>
        <dt>
          <strong
            ><Keyword term="Velocity" /> (<Katex>{"`\\vec{v}`"}</Katex
            >):</strong
          >
        </dt>
        <dd>
          A <Keyword term="Vector" /> quantity representing the rate of change of
          an object's displacement. It includes both speed (its magnitude) and direction.
        </dd>
      </dl>
      <FormulaAccordion title="Velocity Formulas">
        <p>
          <strong
            >Average Velocity (<Katex>{"`\\vec{v}_{avg}`"}</Katex>):</strong
        ...
        <p>
          ...
          >
          The velocity of an object at a specific moment in time (<Katex
            >t</Katex
          >). It's the limit of the average velocity as the time interval <Katex
            >{"`\\Delta t`"}</Katex
          > approaches zero. This is formally defined using calculus as the time
        ...
        <p>
          ...
          problems in introductory kinematics, we'll focus on average velocity
          or cases where instantaneous velocity is constant.
        </p>
      </FormulaAccordion>
      <p>
        <strong>Units:</strong> The standard SI unit for both speed and velocity
        is meters per second (m/s).
      </p>
      <p>
        <strong>Key Distinction:</strong> An object can have a constant speed but
        a changing velocity if its direction changes (like a car driving in a circle
        at a steady 30 km/h).
      </p>
    </ContentCard>
    ...
    <ContentCard blockType="conclusion-block">
      ...
      <p>
        In this section, we established the crucial differences between scalar
        and vector descriptions of motion:
      </p>
      <ul>
        <li>
