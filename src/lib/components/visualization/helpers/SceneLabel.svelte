...
<script lang="ts">
  ...
  let {
    ...
    color = "#ffffff" as Color | string,
    fontSize = 0.5,
    backgroundColor = null as Color | string | null,
    backgroundOpacity = 1.0,
    padding = 0.15, // Padding is relative to fontSize for background sizing
    borderRadius = 0.2, // Absolute radius for corners
    backgroundSmoothness = 4, // Number of segments per corner quarter-circle
  ...
  function createRectangleRounded(
    ...
    for (let j = 0; j < n; j++) {
      ...
      if (current_segment_in_arc <= s) {
        ...
        x = sgx * (w / 2 - r) + r * Math.cos((pi2 * (j - qu + 1)) / (4 * s)); // Fallback to corner logic even if it should be straight
  ...
  const estimatedTextWidth = $derived(
    // Approximate width: number of chars * fontSize * aspect_ratio_char + padding
    text.length * fontSize * 0.55 + padding * 2 * fontSize // Adjusted factor
  );
  const estimatedTextHeight = $derived(fontSize + padding * 2 * fontSize);
  // backgroundDepth removed

  let finalBackgroundColor = $derived(
...
<Billboard position={finalPosition}>
  ...
  {#if finalBackgroundColor && labelBackgroundGeometry}
  ...
  <T.Group>
    <Text {text} {fontSize} {color} {anchorX} {anchorY} {depthTest} {...rest} />
  </T.Group>
</Billboard>
