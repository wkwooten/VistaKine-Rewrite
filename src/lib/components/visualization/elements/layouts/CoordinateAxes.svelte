<script lang="ts">
  ...
  const xLabelWorldPos = cornerOriginOffset.clone().add(new Vector3(axisLengthX + labelOffset, 0, 0));
  const yLabelWorldPos = cornerOriginOffset.clone().add(new Vector3(0, axisLengthY + labelOffset, 0));
  const zLabelWorldPos = cornerOriginOffset.clone().add(new Vector3(0, 0, axisLengthZ + labelOffset));
...
<T.Group position={cornerOriginOffset.toArray()}>
  ...
  {#each gridNumbers as num}
    ...
    <SceneLabel
      position={[num.x, numberYOffset, num.z]} // Positions are relative to the group origin
      text={num.text}
  ...
  {#each yTicks as y (y)}
    <SceneLabel
      position={[-yNumberOffset, y, -yNumberOffset]} // Position relative to group origin
      text={y.toString()}
      fontSize={labelFontSizeGridNum}
      color={yAxisColor}
      anchorX="center"
