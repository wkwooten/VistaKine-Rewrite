<script lang="ts">
  ...
  import {
    Edges,
    Outlines,
  } from "@threlte/extras"; /* Removed Billboard, Text */
  ...
  let {
    color: providedColor, // Renamed from 'color' to distinguish from internal reactive color
    scale = 1,
    controlMode = "drag",
  ...
  $effect(() => {
    ...
    console.log(
      "[Cube.svelte $effect] Syncing scale prop to writableScale:",
      scale
    );
  });
  $effect(() => {
    ...
    console.log(
      "[Cube.svelte $effect] Syncing controlMode prop to writableControlMode:",
      controlMode
    );
  });
  $effect(() => {
    ...
    console.log(
      "[Cube.svelte $effect] Syncing $camera context to writableCamera:",
      $camera
    );
  });
  $effect(() => {
    ...
    console.log(
      "[Cube.svelte $effect] Syncing renderer context to writableRenderer:",
      renderer
    );
  });
  ...
  $effect(() => {
    console.log(
      `[Cube.svelte ${initialPosition.toArray().join(",")}] isSelected derived state changed:`,
      isSelected,
      "$selectedObject:",
      $selectedObject,
      "groupRef:",
  ...
  $effect(() => {
    if (rigidBodyRef)
      console.log(
        "[Cube.svelte $effect] rigidBodyRef prop updated:",
        rigidBodyRef
      );
  });
  $effect(() => {
    console.log(
      "[Cube.svelte $effect] Checking context for Draggable Handlers. Camera:",
      $camera,
      "Renderer:",
      renderer
    );
  ...
  const { handlePointerDown, handlePointerEnter, handlePointerLeave } =
    ...
    if (currentRigidBody) {
      if (currentControlMode === "translate") {
        if (!physicsStateStored) {
          ...
          console.log(
            "[Cube.svelte $effect] Storing physics state:",
            previousRigidBodyTypeStore,
            previousGravityScaleStore
          );
        }
        if (
        ...
        if (physicsStateStored) {
          console.log(
            "[Cube.svelte $effect] Restoring physics state:",
            previousRigidBodyTypeStore,
            previousGravityScaleStore
          );
  ...
  const handleDoubleClick = (e: any) => {
    ...
    console.log(
      `[Cube.svelte ${initialPosition.toArray().join(",")}] Double Click Event Triggered. Move delta: ${moveDelta}`
    );

    if (moveDelta > CLICK_VS_DRAG_THRESHOLD) {
      console.log(
        `[Cube.svelte ${initialPosition.toArray().join(",")}] Double Click ignored, likely end of drag.`
      );
      return;
    }
    ...
    if ($followedObject === groupRef) {
      ...
      console.log(
        `[Cube.svelte ${initialPosition.toArray().join(",")}] Unfollowed. Store value:`,
        $followedObject
      );
    } else {
      followedObject.set(groupRef ?? null);
      console.log(
        `[Cube.svelte ${initialPosition.toArray().join(",")}] Followed. Store value:`,
        $followedObject
      );
    }
  };
  ...
  // Determine the actual color to use for the material
  const cubeMaterialColor = $derived(providedColor ?? $textColor);
</script>
    ...
    <AutoColliders {mass} shape={"cuboid"}>
      <T.Mesh
        ...
        onclick={(e: any) => {
          ...
          const moveDelta = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
          console.log(
            `[Cube.svelte ${initialPosition.toArray().join(",")}] Click Event Triggered. Move delta: ${moveDelta}`
          );

          // If mouse moved significantly, assume it was a drag, not a click
          if (moveDelta > CLICK_VS_DRAG_THRESHOLD) {
            console.log(
              `[Cube.svelte ${initialPosition.toArray().join(",")}] Click ignored, likely end of drag.`
            );
            return;
          }
          ...
          if ($selectedObject === groupRef) {
            ...
            console.log(
              `[Cube.svelte ${initialPosition.toArray().join(",")}] Deselected. Store value after set:`,
              $selectedObject
            );
          } else {
            selectedObject.set(groupRef ?? null); // Select
            console.log(
              `[Cube.svelte ${initialPosition.toArray().join(",")}] Selected. Store value after set:`,
              $selectedObject
            );
          }
        }}
      ...
      >
        ...
        <T.MeshBasicMaterial color={cubeMaterialColor} />
        <Edges color={$accentColor} />
        {#if isSelected || isHovering}
          <Outlines thickness={0.05} color={$accentHoverColor} />
        {/if}
      </T.Mesh>
    </AutoColliders>
  </RigidBody>
